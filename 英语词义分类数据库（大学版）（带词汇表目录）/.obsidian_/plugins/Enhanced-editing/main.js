...
开发：蚕子 QQ：312815311 更新时间：2022-5-17
***************************************************************************** */
...
class MyPlugin extends obsidian.Plugin {
        ...
        this.registerDomEvent(this.statusBarIcon, "click", (e) => {
            ...
            */
            ...
            menu.addItem((item) => {
                ...
                item.onClick(() => this.彩字格式刷(this.settings.bColor1,'荧光笔1'));
            });
            menu.addItem((item) => {
                item.setIcon("格式刷2");
                item.setSection("highlight_html");
                item.onClick(() => this.彩字格式刷(this.settings.bColor2,'荧光笔2'));
            });
            menu.addItem((item) => {
                item.setIcon("格式刷3");
                item.setSection("highlight_html");
                item.onClick(() => this.彩字格式刷(this.settings.bColor3,'荧光笔3'));
            });
            menu.addItem((item) => {
                item.setIcon("格式刷4");
                item.setSection("highlight_html");
                item.onClick(() => this.彩字格式刷(this.settings.bColor4,'荧光笔4'));
            });
            menu.addItem((item) => {
                item.setIcon("格式刷5");
                item.setSection("highlight_html");
                item.onClick(() => this.彩字格式刷(this.settings.bColor5,'荧光笔5'));
            });
    ...
    修复错误标点() {
        this.获取编辑器信息 ();
        if (!所选文本) return;
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s*,\s*(?=[一-龥])/g,"，");
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s*\s*\.(?=[一-龥])/g,"。");   //数字后的黑点不转为句号
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s*\?\s*(?=[一-龥])/g,"？");
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s*:\s*(?=[一-龥])/g,"：");
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s*;\s*(?=[一-龥])/g,"；");
        所选文本 = 所选文本.replace(/(?<=[一-龥])\s+(?=[一-龥])/g,"");
        所选文本 = 所选文本.replace(/(?<=[^一-龥])，(?=[^一-龥])/g,",");
        所选文本 = 所选文本.replace(/(?<=[^一-龥])。(?=[^一-龥])/g,".");   //数字后的黑点不转为句号
        所选文本 = 所选文本.replace(/(?<=[^一-龥])？(?=[^一-龥])/g,"?");
        所选文本 = 所选文本.replace(/(?<=[^一-龥])：(?=[^一-龥])/g,":");
        所选文本 = 所选文本.replace(/(?<=[^一-龥])；(?=[^一-龥])/g,";");
        所选文本 = 所选文本.replace(/[\(（)]([^一-龥]+)[\)）]/g,"($1)");
        所选文本 = 所选文本.replace(/[\(（)](.*?[一-龥].*?)[\)）]/g,"（$1）");
        所选文本 = 所选文本.replace(/：：/g,"::");
        所选文本 = 所选文本.replace(/【【/g,"[[");
        所选文本 = 所选文本.replace(/】】/g,"]]");
        所选文本 = 所选文本.replace(/(?<=[,\.?!])([^\s])/g," $1");
        this.替换所选文本 (所选文本);
    };
...
class editSettingsTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new obsidian.Setting(containerEl)
            ...
            .setDesc("支持批量转换多行文本（需用换行符分隔）或多句文本（需用顿号分隔）。")
        ...
        */
        ...
        toolText.appendChild(document.createElement('br'));
        toolText.appendText('获取当前字符数「未设置」∶计算当前笔记中可见字符（汉字、字母、数字、标点）个数和不可见字符（空格、换行符）个数');
        toolText.appendChild(document.createElement('br'));
